...
```bash
bash codex/download.sh
bash codex/wrapper.sh
export PERSISTENT_CODEX_HOME=/path/to/persistent/.codex
bash codex/link.sh "$PERSISTENT_CODEX_HOME"
```
...
export PERSISTENT_CODEX_HOME=/path/to/persistent/.codex
bash /path/to/codex-server-setup/codex/link.sh "$PERSISTENT_CODEX_HOME"
```
...
```bash
PERSISTENT_CODEX_HOME=/path/to/persistent/.codex bash codex/link.sh
```
...
```bash
CODEX_INSTALL_PATH=/usr/local/bin/codex bash codex/download.sh
CODEX_BIN=/usr/local/bin/codex CODEX_REAL=/usr/local/bin/codex-real bash codex/wrapper.sh
```
...
```bash
CODEX_HOME="$HOME/.codex" bash codex/link.sh /path/to/persistent/.codex
```

如果也要用 `setup.sh` 一键初始化：

```bash
CODEX_HOME="$HOME/.codex" bash codex/setup.sh /path/to/persistent/.codex
```
